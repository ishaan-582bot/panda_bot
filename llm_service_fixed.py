...
class LLMService:
    ...
    async def generate_response(
        ...
        # OpenAI provider - only use fallback if NO chunks available at all (not just below threshold)
        if not context_chunks:
            logger.warning("No context chunks available for query - index may be empty")
            return (
    ...
    def _calculate_confidence(
        ...
        # Lowered thresholds for local embedding models and small datasets
        if avg_score > 0.5 and is_valid:
            return "high"
        elif avg_score > 0.25:
            return "medium"
