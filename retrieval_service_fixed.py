...
class RetrievalService:
    ...
    async def search_with_threshold(
        ...
        For small datasets: always return best results, just label confidence accordingly.
        """
        ...
        # Dynamic threshold calibration for small datasets
        ...
        if all_scores:
            ...
            if results[0][1] < min_score and len(results) > 1:
                ...
                if score_gap > 0.1:
                    adjusted_threshold = min_score * 0.5  # More aggressive lowering
        ...
        # CRITICAL FIX: For small datasets, return top results even if below threshold
        # Only return empty if scores are extremely low (below 0.1 floor)
        ABSOLUTE_FLOOR = 0.1
        
        if results and results[0][1] < min_score:
            if results[0][1] >= ABSOLUTE_FLOOR:
                # Return best results with warning - let confidence label handle the quality indicator
                logger.info(f"Top result score {results[0][1]:.2f} below threshold {min_score}, "
                           f"but above floor {ABSOLUTE_FLOOR}. Returning best matches.")
                return results[:top_k] if top_k else results
            else:
                logger.info(f"Top result score {results[0][1]:.2f} below absolute floor {ABSOLUTE_FLOOR}")
                return []
